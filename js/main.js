$(document).ready(function(){
	var DOMAIN = "https://britishcollegeproject-444.000webhostapp.com";
...
// registration validation process
	$("#register_form").on("submit",function(){
		...
		if(name.val() == "" || name.val().length < 6){
			...
			$("#u_error").html("<span class='text-danger'>Please enter full name and should be more than 5 char</span>");
			status = false;
		}else{
		...
		if(type.val() == ""){
			...
			$("#t_error").html("<span class='text-danger'>Please select any one option.</span>");
			status = false;
		}else{
			...
			$.ajax({
				...
				success : function(data){
					if (data == "EMAIL_ALREADY_EXISTS") {
						...
						$(".overlay").hide();
						$("#display_msg").html("Congratulations! You are registered. Now you can login");
						setTimeout(function(){ window.location.href = encodeURI(DOMAIN+"/login.php"); }, 3000);
