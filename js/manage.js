$(document).ready(function(){
	...
	function searchLive(searchValue){
		$.ajax({
			url : DOMAIN+"/includes/process.php",
			method : "POST",
			data : {searchLive:searchValue},
			success : function(output){
				if (output == "NO_DATA_FOUND") {
						alert("Sorry ! Record not found");
					}else{
						$("#get_product").html('');
						$("#get_product").html(output);
					}		
			}
		})
	}
			...
			$.ajax({
				...
				success : function(data){
					if (data == "DEPENDENT_CATEGORY") {
						...
						$("#display_msg").html("<span class='text-success'>Category Deleted Successfully</span>");
						manageCategory(1);
