...
class DBOperation
{
	...
	public function searchAllRecord($param){
		$pre_stmt = $this->con->prepare("SELECT p.pid,p.product_name,c.category_name,b.brand_name,p.product_price,p.product_stock,p.added_date,p.p_status FROM products p,brands b,categories c WHERE ((p.bid = b.bid AND p.cid = c.cid) AND (p.product_name like ? OR c.category_name like ? OR b.brand_name like ?))");
		$paramaValue = '%'.$param.'%';
		$pre_stmt->bind_param("sss", $paramaValue, $paramaValue, $paramaValue);
		$pre_stmt->execute() or die($this->con->error);
		$result = $pre_stmt->get_result();
		$rows = array();
		if ($result->num_rows > 0) {
