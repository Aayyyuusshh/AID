...
//for live search
if (isset($_POST["searchLive"])) {
	$paramValue = $_POST["searchLive"];
	$dbop = new DBOperation();
	$result = $dbop->searchAllRecord($paramValue);
	if (is_array($result)) {
		# code...
		$rows = $result["rows"];
		if (count($rows) > 0) {
...
if (isset($_POST["getNewOrderItem"])) {
	...
	?>
	<tr>
		<td><button id="btn_remove" class="fa fa-trash" onclick="$(this).parent().parent().remove();"></button></td>
